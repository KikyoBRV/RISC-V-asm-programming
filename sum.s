...
#        arr1[i] = i + 1;
    ...
    addi x8, x0, 0 # let x8 be the i and set it to 0
    la x9, arr1 # loading the address of arr1 to x9
    
    
loop1: 
    bge x8, x5, exit1 # checl if i >= size, if so goto exit1
    # we need to calculate &arr1[i]
    # we need the base address of arr1
    # then, we add an offset of i*a to the base address
...
loop2: 
    bge x8, x5, exit2 # checl if i >= size, if so goto exit1
    # we need to calculate &arr2[i]
    # we need the base address of arr1
    # then, we add an offset of i*a to the base address
    slli x18, x8, 2 # set x18 to i*4
    add x19, x18, x21 # add i*4 to the base address off arr1 and put it to x19
